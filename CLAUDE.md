...
- **`guard.ts`** — the lexical guard (`inspectSql`): one statement only, allow-listed leading keyword, comment/string-aware. It also rejects `set_config()`, which is **not** a stylistic rule: `set_config('role', ...)` is an ordinary function call, so it rides inside a permitted `SELECT`/`WITH`, and Postgres checks role membership against `session_user` — the shared bootstrap role, a member of every tenant — rather than against whatever `SET LOCAL ROLE` made current. Without that check, one query escapes its tenant. It is rejected wholesale rather than by inspecting which parameter it targets, because the first argument can be an arbitrary expression.
...
5. `inspectSql()` in `guard.ts` — single-statement, allow-listed leading keyword, no `set_config()` (the only way user SQL can re-enter a different role mid-transaction) — plus the extended protocol, which rejects multi-statement text at the wire level.
