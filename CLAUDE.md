...
- A tenant role must have `USAGE` on exactly one non-system schema. Zero or more than one makes `resolveSchema()` throw, which surfaces as a login failure — that's intentional, not a gap to paper over with a config override.
